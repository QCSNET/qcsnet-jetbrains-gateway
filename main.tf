...
variable "ide_path_on_host" {
  type        = string
  description = "Percorso remoto per l'IDE. Se non specificato, il plugin imposterà '.cache'."
  default     = "/workspaces"
}
...
resource "coder_app" "gateway" {
  ...
  url = join("", [
    ...
    "&token=", "$SESSION_TOKEN",
    "&ide_product_code=", data.coder_parameter.jetbrains_ide.value,
    "&ide_build_number=", local.build_number,
    "&ide_download_link=", local.download_link,
    var.ide_path_on_host != "" ? "&ide_path_on_host=" : "",
    var.ide_path_on_host != "" ? var.ide_path_on_host : ""
  ])
}
